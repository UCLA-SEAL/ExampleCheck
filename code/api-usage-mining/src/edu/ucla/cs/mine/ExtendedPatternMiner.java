...
public class ExtendedPatternMiner {
	public static Map<ArrayList<APISeqItem>, MutablePair<Double, Double>> mine(
		...
		SequencePatternMiner pm = new ExtendedFrequentSequenceMiner(
				"/home/troy/SO-Empirical-Study/code/Maple/mining/freq_seq.py", sequence,
		...
		for (ArrayList<String> seq_pattern : patterns.keySet()) {
			PredicatePatternMiner pm2 = new ExtendedPredicateMiner(
		...
		if(apis.size() == 1) {
			...
			ArrayList<ArrayList<APISeqItem>> remove = new ArrayList<ArrayList<APISeqItem>>();
			for(ArrayList<APISeqItem> pattern : ranked_patterns.keySet()) {
				if(pattern.size() == query.size()) {
					boolean flag = true;
					for(APISeqItem item : pattern) {
						if(item instanceof ExtendedAPICall && !((ExtendedAPICall)item).condition.equals("true")) {
							flag = false;
						}
					}
			...
			// check for patterns about overridden APIs, because we want to know which API is used more often
			ArrayList<ArrayList<APISeqItem>> overridenAPIPatterns = new ArrayList<ArrayList<APISeqItem>>();
			for(int i = 0; i < remove.size(); i++) {
				ArrayList<APISeqItem> p1 = remove.get(i);
				if(p1.size() == 1) {
					String api = ((ExtendedAPICall)p1.get(0)).name;
					String name = api.substring(0, api.indexOf('('));
					String args = api.substring(api.indexOf('(') + 1, api.indexOf(')'));
					for(int j = i + 1; j < remove.size(); j++) {
						ArrayList<APISeqItem> p2 = remove.get(j);
						if(p2.size() == 1) {
							String api2 = ((ExtendedAPICall)p2.get(0)).name;
							String name2 = api2.substring(0, api2.indexOf('('));
							String args2 = api2.substring(api2.indexOf('(') + 1, api2.indexOf(')'));
							if(name.equals(name2) && !args.equals(args2)) {
								overridenAPIPatterns.add(p1);
								overridenAPIPatterns.add(p2);
								break;
							}
						}
					}
			...
			for(ArrayList<APISeqItem> pattern : overridenAPIPatterns) {
				remove.remove(pattern);
			}
			
			for(ArrayList<APISeqItem> r : remove) {
				ranked_patterns.remove(r);
			}
		}
	...
	private static HashMap<ArrayList<APISeqItem>, MutablePair<Double, Double>> compose(
		...
		for (String api : seq_pattern) {
			if (api.equals("}")) {
			...
			} else if (api.contains("CATCH(") && api.endsWith("Exception) {")) {
				String exceptionType = api.substring(api.indexOf('(') + 1, api.indexOf(')'));
				CATCH catchBlock = new CATCH(exceptionType);
				if (composed_patterns.isEmpty()) {
					// this is the first element in the sequence pattern
					ArrayList<APISeqItem> s = new ArrayList<APISeqItem>();
					s.add(catchBlock);
				...
				// split the name and the argument types of the API call
				String name = api.substring(0, api.indexOf('('));
				
				HashMap<String, Integer> conditions;
				if (pred_patterns.containsKey(name)) {
					conditions = pred_patterns.get(name);
				...
				if (composed_patterns.isEmpty()) {
					...
					for (String condition : conditions.keySet()) {
						// initialize
						ArrayList<APISeqItem> s = new ArrayList<APISeqItem>();
						s.add(new ExtendedAPICall(api, condition));
					...
					for (Map.Entry<ArrayList<APISeqItem>, MutablePair<Double, Double>> entry : composed_patterns
						...
						for (String condition : conditions.keySet()) {
							...
							newS.addAll(s);
							newS.add(new ExtendedAPICall(api, condition));
	...
	public static void sample(String seqFile, String orgFile, int size, Map<ArrayList<APISeqItem>, MutablePair<Double, Double>> patterns, String dest, int n) {
		...
		for (ArrayList<APISeqItem> pattern : patterns.keySet()) {
			...
			// find n sample examples of this pattern
	...
	public static void main(String[] args) {
		// reset the template file name to avoid conflicts
		SAT.temp = "JsonElement.getAsString.txt";
		String raw_output = "/media/troy/Disk2/Boa/apis/JsonElement.getAsString/1-clean.txt";
		String seq = "/media/troy/Disk2/Boa/apis/JsonElement.getAsString/large-output-resolved.txt";
		...
		System.out.println("Total Code Examples: " + size);
